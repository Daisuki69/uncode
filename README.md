...
29. **Consolidated Web Architecture (Pure-List `KnownDistractingWeb`, `KnownSafeWeb` & Flowchart Truth Table Realignment)**:
    ...
    - **Proxy & Cloud-Rendered Browser Neutralization & 1DM+ Integration**:
      - **Why this was changed**:
        - *Proxy & Onion Browser Loophole*: Cloud-rendered and onion routing browsers (Puffin, Tor Browser, UC Browser) bypass both Layer 1 Local DNS Sinkhole (Puffin renders in cloud servers without device DNS; Tor uses internal SOCKS5 proxy) and Layer 2 Accessibility Auto-Back (hardened/OpenGL render trees). Because Tor's package `org.torproject.torbrowser` contains `"browser"`, `isBrowserPackage()` previously allowed it to run uninspected.
        - *1DM+ Pro Omission*: 1DM+ (`idm.internet.download.manager.plus`) was not matched by exact string `"idm.internet.download.manager"`, risking false-positive app block.
      - **Concrete Architectural Fixes Implemented**:
        - **Proxy Browser Blacklist (`KnownDistracting.kt`)**: Added Tor Browser (`org.torproject.torbrowser`), Puffin (`com.cloudmosa.puffinfree`, `com.cloudmosa.puffin`), and UC Browser (`com.ucmobile.intl`, `com.uc.browser.en`) directly into `KnownDistracting.kt`.
        - **Browser Package Master Veto (`LockAccessibilityService.java`)**: Preempted `isBrowserPackage()` with `if (KnownDistracting.isKnownDistracting(pkg)) return false;`. Proxy browsers are never treated as standard web browsers; they are blocked and evicted directly to QIEZKA at Stage 2.
        - **1DM Family Wildcard Match**: Expanded `isBrowserPackage()` to match `pkg.startsWith("idm.internet.download.manager")` and added `pkg + ":id/search"` into `extractUrlFromBrowser()`, ensuring 1DM+ Pro is 100% recognized, DNS-sinkholed, and Auto-Backed.
    - **LocalDnsVpnService Teardown Synchronization & Idle-Time DNS Passthrough**:
      - **Why this was changed**:
        - *Zombie VPN OS-Binding Deadlock*: Under Option B (Session-Only Web Guard), `LocalDnsVpnService` is strictly intended to run only during active focus sessions (`shouldVpnRun = isVpnMode && isEnforcing`). When a session ended or when the system was idle (no active lockdown, consequence mode, or schedule), `LockAccessibilityService` called `LocalDnsVpnService.stopVpn()`, which executed `context.stopService()`. However, because Android OS (`system_server`) held an active Binder connection to `LocalDnsVpnService` while `vpnInterface` (`tun0`) remained open, calling `stopService()` could not destroy the service. Teardown logic (`stopDnsSinkhole()`) was only in `onDestroy()`, which was deadlocked and never called. Consequently, `tun0` stayed open, `isRunning` remained `true`, and all UDP port 53 DNS queries on the device continued to be intercepted and sinkholed even during free time.
        - *AI Assistant False-Positive Block During Free Time*: Because `chatgpt.com` was registered in `KnownDistractingWeb.AI_ASSISTANT_DOMAINS` and `handleDnsPacket()` evaluated `WebClassifier` unconditionally without verifying session enforcement, ChatGPT was sinkholed with NXDOMAIN (`AI assistants are blocked during focus mode`) during total system idle whenever "Allow AI" was toggled off.
      - **Concrete Architectural Fixes Implemented**:
        - **`ACTION_STOP` Intent Action & Lifecycle Teardown (`LocalDnsVpnService.java`)**: Added `ACTION_STOP` (`"com.uncode.app.ACTION_STOP_VPN"`) and a thread-safe static reference `sInstance`. When `stopVpn()` is called, it immediately executes synchronous instance teardown (`sInstance.stopDnsSinkhole()` and `sInstance.stopSelf()`), dispatches `ACTION_STOP`, and calls `context.stopService()`. In `onStartCommand()`, receiving `ACTION_STOP` cleanly calls `stopDnsSinkhole()` and `stopSelf()`, ensuring `vpnInterface.close()` is executed immediately, releasing the OS Binder connection and destroying `tun0`.
        - **Defense-in-Depth Idle Passthrough (`LocalDnsVpnService.java`)**: In `handleDnsPacket()`, added a pre-filter check for `isEnforcing = isLockdownActive || isConsequenceActive`. If the system is in idle free time, domain blocking heuristics are bypassed completely and queries are forwarded directly to upstream resolvers (`1.1.1.1` / `8.8.8.8`), ensuring zero websites are sinkholed when focus enforcement is not active.
        - **Protection Mode Switch Cleanup (`LockPlugin.java`)**: Updated `setWebProtectionMode()` to explicitly trigger `LocalDnsVpnService.stopVpn()` when switching protection modes while idle, preventing lingering VPN interfaces.
    - **ForegroundServiceDidNotStartInTimeException Resolution in LocalDnsVpnService Teardown**:
      - **Why this was changed**:
        - *Android Foreground Service Contract Violation*: When testing mode changes on Android (specifically Evolution X / Android 14+), the system crashed with `android.app.RemoteServiceException$ForegroundServiceDidNotStartInTimeException: Context.startForegroundService() did not then call Service.startForeground()`.
        - *Root Cause Analysis*: In `LocalDnsVpnService.stopVpn()`, `context.startForegroundService(intent)` was erroneously called to dispatch an `ACTION_STOP` intent to stop the service. Under Android's strict background execution limits (API 26+), `startForegroundService()` establishes a mandatory contract requiring the service to call `Service.startForeground()` with an ongoing notification within 5-10 seconds. Because `onStartCommand()` immediately executed `stopSelf()` without calling `startForeground()`, the Android OS threw `ForegroundServiceDidNotStartInTimeException` and killed the app process.
      - **Concrete Architectural Fixes Implemented**:
        - **Eliminated `startForegroundService()` from Service Teardown (`LocalDnsVpnService.java`)**: Removed all invocations of `startForegroundService()` from `stopVpn()`. Foreground service teardown strictly relies on direct synchronous instance termination (`sInstance.stopDnsSinkhole()` and `sInstance.stopSelf()`) followed by standard `context.stopService(stopIntent)`, which has no `startForeground()` obligation.
        - **Notification Update Contract Compliance**: In `onStartCommand()`, wrapped `ACTION_UPDATE_NOTIF` handling to invoke `startForeground(NOTIF_ID, notif)` to guarantee contract satisfaction if triggered via external foreground intents, and enabled direct notification dispatch when `sInstance != null`.
    - **Educational Flashcard & Quiz Platforms Integration (Gizmo, Study Tools)**:
      - **Why this was changed**:
        - *Gizmo and Quiz App Exclusion from Allowed Applications*: When testing on device, the Gizmo Quiz App (**Gizmo: AI Flashcards and Tutor**) was not listed under Allowed Applications in the Dashboard selection dialog and could not be whitelisted.
        - *Root Cause Analysis*:
          1. Gizmo's official Google Play package identifier is `ai.saveall.app` (developed by Save All Ltd), with an alternate package variant `com.gizmo.ai.flashcards`.
          2. Android OS lacks a `CATEGORY_EDUCATION` constant in `ApplicationInfo`; Gizmo and many third-party study apps default to `CATEGORY_UNDEFINED`.
          3. `AppClassifier.POSITIVE_ACADEMIC_KEYWORDS` lacked keywords like `"quiz"`, `"quizzes"`, `"tutor"`, `"exam"`, `"testprep"`, and `"gizmo"`. Because neither `ai.saveall.app` nor label `"Gizmo"` contained the existing keyword `"flashcards"`, it fell into Layer 5 Fallback (`isPackageBlocked = true`).
          4. In `LockPlugin.getInstalledApps()`, `AppClassifier.isPackageBlocked() == true` caused Gizmo to be skipped from the UI. Furthermore, because Gizmo has "AI" in its title/package, it was at risk of exclusion by `isAiAppKeywords()` unless protected by baseline safety.
      - **Concrete Architectural Fixes Implemented**:
        - **Formal Baseline Invariant Inclusion (`KnownSafe.kt`)**: Added `ai.saveall.app` and `com.gizmo.ai.flashcards` (Gizmo), `com.cram.cramandroid` (Cram), `com.brainscape.mobile.portal` (Brainscape), `com.studysmarter` / `de.studysmarter` (StudySmarter / Vaia), `com.kahoot.android` (Kahoot!), `com.quizizz.android` (Quizizz), and `com.magoosh.flashcards.gre` / `com.magoosh.vocab.gre` (Magoosh) directly into `BASELINE_SAFE_PACKAGES`.
...
30. **Home Shell Foundation: Dual App Icons & MainActivity Liveness Delegation**:
    ...
    - **Settings & Fallback Launcher Disambiguation (`com.android.settings.FallbackHome` Elimination)**:
      - **Why this was changed (Root Cause Analysis & Forensic Breakdown)**:
        - *The "Settings as Launcher" Bug on Physical Devices*: During real-world testing across two separate physical Android phones, `com.android.settings` appeared in the Detected Home Launchers inventory card in Settings and was selected as a candidate Home launcher.
        - *AOSP Direct Boot Mechanics*: Android OS / AOSP ships an internal activity inside the Settings APK: `com.android.settings.FallbackHome`. It declares `<category android:name="android.intent.category.HOME" />` and `android:priority="-1000"`, functioning as a placeholder during Direct Boot (File-Based Encryption before initial PIN/pattern unlock) or when no launcher is installed.
        - *Intent Query Collision & Cascading Impact*:
          1. Because `InstalledLauncherDetector.java` queried `CATEGORY_HOME` without negative priority or settings exclusions, `com.android.settings` was presented in the UI as an installed launcher.
          2. In `QiezkaHomeHandlerActivity.java`, `getSelectedLauncherComponent()` could auto-select and persist `com.android.settings` to `qiezka_home_proxy` SharedPreferences, causing Home delegations to launch Settings.
          3. In `LockAccessibilityService.java`, `resolveDynamicExemptions()` added `com.android.settings` to `dynamicLauncherPackages`, which could interfere with accessibility lockdown enforcement.
      - **Concrete Architectural Fixes Implemented**:
        - **Blacklisted Launcher Package Filter (`InstalledLauncherDetector.java`)**: Added `isBlacklistedLauncherPackage(pkg)` checking `com.android.settings`, packages containing `.settings`, `android`, `com.android.systemui`, setup wizards (`setupwizard`), provisioning tools (`com.android.provision`, `smartswitch`, `easymover`), and delegating to `AppClassifier.isSettingsOrDeviceManager(pkg, "")`.
        - **Real Launcher Validation & Priority Inspection (`InstalledLauncherDetector.java`)**: Added `isRealLauncher(info, myPkg)` which rejects self (`com.uncode.app`), vetoes blacklisted packages, rejects activities with `info.priority < 0` (filtering AOSP `FallbackHome` `-1000`), and rejects activity names matching `FallbackHome` or `SetupWizard`. Applied to candidate queries and default resolver checks.
        - **Persistent SharedPreferences Sanitization (`QiezkaHomeHandlerActivity.java`)**: In `getSelectedLauncherComponent()`, if `savedPkg` is `com.android.settings` or blacklisted, it is automatically purged from `qiezka_home_proxy` SharedPreferences, and candidate discovery strictly selects launchers vetted by `isRealLauncher()`.
        - **Lock Accessibility Guard (`LockAccessibilityService.java`)**: In `isLauncherApp()`, added an immediate veto against `isBlacklistedLauncherPackage(pkg)` and ensured `dynamicLauncherPackages` only caches genuine launchers vetted by `isRealLauncher()`.
        - **Unit Test Coverage (`HomeHandlerTest.kt`)**: Added `testBlacklistedLauncherPackages()` and `testIsRealLauncherFiltersSettingsAndFallbacks()` verifying rejection of `com.android.settings`, `FallbackHome`, setup wizards, and negative priority fallbacks, while preserving genuine launchers (One UI, Nexus Launcher, Lawnchair, Nova).
    - **Single-Source-of-Truth Architectural Unification: Stage 1 Master Veto for Launcher Discovery**:
      - **Why this was changed (Architectural Alignment & Elimination of Duplicate Blacklists)**:
        - *Eliminating Duplicate Filter Fragmentation*: Having an ad-hoc `isBlacklistedLauncherPackage` method in `InstalledLauncherDetector.java` duplicated package string checks (`com.android.settings`, `.settings`, `setupwizard`, etc.) that were already maintained in `AppClassifier.java`.
        - *Single Source of Truth*: Under QIEZKA's 3-Stage Model, **Stage 1 Master Veto Gate** (`AppClassifier.isStage1Vetoed()`) is the universal, authoritative filter for prohibited system components (Settings, Device Care/Managers, Setup Wizards, Bloatware, and Known Distractions).
        - *Why Only Stage 1*: Candidate launchers are evaluated strictly through Stage 1 Master Veto rather than full Stage 3. In Stage 3, Layer 5 Fallback is a conservative whitelist designed to block unknown user-installed apps without academic tokens; third-party launchers (Nova, Lawnchair) are installed on the data partition and are permitted *because* they are launchers. Furthermore, full Stage 3 evaluation would introduce circular dependency with `isLauncherApp()`.
      - **Concrete Architectural Fixes Implemented**:
        - **Consolidated Anti-Tamper & Setup Wizards in `AppClassifier.java`**: Added setup wizards (`setupwizard`), device provisioning (`com.android.provision`, `smartswitch`, `easymover`), `android`, and `com.android.systemui` directly into `isSettingsOrDeviceManager()`.
        - **Unified `isStage1Vetoed()` Gate (`AppClassifier.java`)**: Exposed a public static helper `isStage1Vetoed(pkg, appLabel)` unifying `isSettingsOrDeviceManager()`, `isStage1Bloat()`, and `KnownDistracting.isKnownDistracting()`.
        - **Eliminated Ad-Hoc Blacklist (`InstalledLauncherDetector.java`)**: Removed `isBlacklistedLauncherPackage()`. `isRealLauncher()` now directly routes candidate packages through `AppClassifier.isStage1Vetoed()`, preserving Android manifest invariants (`priority >= 0`, `!FallbackHome`, self-exclusion).
        - **Consolidated Callers (`LockAccessibilityService.java` & `QiezkaHomeHandlerActivity.java`)**: Replaced all references to the old blacklist with `AppClassifier.isStage1Vetoed()`.
        - **Unit Test Coverage (`HomeHandlerTest.kt`)**: Added `testStage1MasterVetoForLaunchers()` verifying that Stage 1 Master Veto rejects settings, setup wizards, bloatware, and distractions, while allowing genuine OEM and third-party launchers.
