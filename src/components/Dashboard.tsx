...
import { Clock, BookOpen, Trash2, Plus, Sparkles, Pencil, Upload, Loader2, Settings, ShieldAlert, X, GitMerge, FileText, Calculator, Music, Globe, MessageSquare, MonitorPlay, Check, LayoutGrid, Camera, ShieldCheck, AlertTriangle, RotateCcw, Lock } from 'lucide-react';
import { AppSettings, SavedResource, ScheduleData, AllowedApp, PunishmentState } from '../types';
...
export function Dashboard({ 
  ...
  return (
    <div className="relative w-full h-full flex-1 overflow-hidden">
            ...
            <button
              onClick={() => {
                if (punishment?.isActive) {
                  showError("In-App Settings is locked while Study Detention is active. Pass your failed homework to unlock.");
                  return;
                }
                onOpenSettings();
              }}
              className={`p-2 bg-white border border-gray-200 rounded-lg transition-colors ${
                punishment?.isActive 
                  ? 'opacity-60 cursor-not-allowed text-red-500 bg-red-50' 
                  : 'hover:bg-gray-50 text-gray-500'
              }`}
              title={punishment?.isActive ? "Settings Locked (Study Detention Active)" : "Settings"}
            >
              {punishment?.isActive ? <Lock className="w-5 h-5" /> : <Settings className="w-5 h-5" />}
            </button>
          </div>
        </header>

        {punishment?.isActive && (
          <div className="mb-6 bg-gradient-to-r from-red-950 to-red-900 border-2 border-red-600 text-white rounded-3xl p-5 shadow-xl animate-in fade-in slide-in-from-top-4">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="font-black text-base text-white tracking-wide uppercase">
                      Study Detention Active
                    </h3>
                    <span className="text-[10px] font-black uppercase bg-red-800 border border-red-500 text-red-100 px-2 py-0.5 rounded-md">
                      10 Apps + Settings Blocked
                    </span>
                  </div>
                  <p className="text-xs text-red-200 mt-1 leading-relaxed">
                    Homework <strong className="text-white">"{punishment.scheduleTitle || 'Homework Session'}"</strong> expired without submission. 10 of your most used apps and Android Settings are locked.
                  </p>
                  <p className="text-[11px] text-red-300/90 mt-0.5">
                    To unblock: go to <strong>Failed Homeworks</strong>, tap <strong>"Add Again (Reschedule)"</strong>, and pass the evaluation.
                  </p>
                  <div className="mt-3.5 flex items-center gap-2 flex-wrap">
                    <button
                      onClick={onOpenLogs}
                      className="px-4 py-2 bg-white text-red-700 hover:bg-red-50 text-xs font-black rounded-xl shadow-md transition-colors flex items-center gap-1.5"
                    >
                      <RotateCcw className="w-3.5 h-3.5" />
                      Reschedule in Failed Homeworks
                    </button>
                    {punishment.punishedPackages && punishment.punishedPackages.length > 0 && (
                      <button
                        onClick={() => setShowBlockedAppsModal(true)}
                        className="px-3.5 py-2 bg-red-900/90 hover:bg-red-800 text-red-200 hover:text-white border border-red-700 text-xs font-bold rounded-xl transition-colors"
                      >
                        View {punishment.punishedPackages.length} Blocked Apps
                      </button>
                    )}
                  </div>
                </div>
      ...
      {/* Study Detention Blocked Apps Modal */}
      {showBlockedAppsModal && punishment?.isActive && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-100 flex flex-col max-h-[85vh] animate-in fade-in zoom-in-95 duration-150">
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-red-50/80">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-red-600 text-white flex items-center justify-center shadow-xs">
                  <ShieldAlert className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-bold text-gray-900 text-base">Locked Under Detention</h3>
                  <p className="text-xs text-red-700 font-semibold">{punishment.punishedPackages.length} Apps + Android Settings</p>
                </div>
              </div>
              <button
                onClick={() => setShowBlockedAppsModal(false)}
                className="p-2 bg-white/80 hover:bg-white rounded-full text-gray-400 hover:text-gray-700 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-gray-50 space-y-2">
              <p className="text-xs text-gray-500 mb-3 px-1">
                These applications are actively blocked on your device until you reschedule and pass <strong>"{punishment.scheduleTitle || 'your homework'}"</strong>.
              </p>
              {punishment.punishedAppDetails && punishment.punishedAppDetails.length > 0 ? (
                punishment.punishedAppDetails.map((app) => (
                  <div key={app.id} className="flex items-center gap-3 p-2.5 bg-white rounded-xl border border-red-100 shadow-2xs">
                    {app.iconBase64 ? (
                      <img src={app.iconBase64} alt={app.name} className="w-9 h-9 rounded-xl object-contain" />
                    ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="font-bold text-sm text-gray-900 truncate">{app.name}</div>
                      <div className="font-mono text-[10px] text-gray-400 truncate">{app.id}</div>
                    </div>
                    <span className="text-[10px] uppercase font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-md border border-red-200">
                      Locked
                    </span>
                  </div>
                ))
              ) : (
                punishment.punishedPackages.map((pkg) => (
                  <div key={pkg} className="flex items-center justify-between p-2.5 bg-white rounded-xl border border-red-100 shadow-2xs">
                    <span className="font-mono text-xs text-gray-800 truncate pr-2">{pkg}</span>
                    <span className="text-[10px] uppercase font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-md border border-red-200 shrink-0">
                      Locked
                    </span>
                  </div>
                ))
            ...
            <div className="p-4 bg-white border-t border-gray-100 flex justify-between items-center gap-2">
              <button
                onClick={() => {
                  setShowBlockedAppsModal(false);
                  onOpenLogs();
                }}
                className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white font-bold text-xs rounded-xl transition-colors flex items-center gap-1.5 shadow-xs"
              >
                <RotateCcw className="w-3.5 h-3.5" />
                Go to Failed Homeworks
              </button>
              <button
                onClick={() => setShowBlockedAppsModal(false)}
